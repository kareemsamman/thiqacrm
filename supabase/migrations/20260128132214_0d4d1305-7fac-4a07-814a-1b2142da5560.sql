-- Drop and recreate with fixed UUID aggregation
DROP FUNCTION IF EXISTS public.report_created_policies;
DROP FUNCTION IF EXISTS public.report_renewals;

-- Recreate report_created_policies with fixed UUID handling
...
BEGIN
  ...
  WITH grouped_policies AS (
    SELECT 
      ...
      -- For package: use first policy's data (by policy_type order)
...
-- Recreate report_renewals with fixed UUID handling
...
BEGIN
  ...
  -- Get total count first (renewals = end_date within range)
  SELECT COUNT(DISTINCT COALESCE(p.group_id::text, p.id::text))
  INTO v_total
  FROM policies p
  JOIN clients c ON c.id = p.client_id
