        ...
        <div className="w-full max-w-md animate-scale-in">
          ...
          <div className="rounded-3xl border border-white/20 bg-white/70 dark:bg-card/70 backdrop-blur-xl shadow-2xl shadow-black/10 overflow-hidden">
            ...
            <div className="text-center pt-10 pb-4 px-8">
              <img src={logoSrc} alt={siteTitle} className="mx-auto h-14 w-14 object-contain lg:hidden mb-4" />
            ...
            <div className="px-8 pb-10 space-y-5">
              ...
              <Tabs defaultValue="google" className="w-full">
                <TabsList className="grid w-full grid-cols-3 bg-white/50 dark:bg-muted/50 backdrop-blur-sm rounded-xl h-11 p-1">
                  <TabsTrigger value="google" className="text-xs sm:text-sm rounded-lg data-[state=active]:bg-white dark:data-[state=active]:bg-card data-[state=active]:shadow-sm">Google</TabsTrigger>
                  <TabsTrigger value="email" className="text-xs sm:text-sm gap-1 rounded-lg data-[state=active]:bg-white dark:data-[state=active]:bg-card data-[state=active]:shadow-sm">
                    <Mail className="h-3.5 w-3.5 hidden sm:block" />بريد
                  </TabsTrigger>
                  <TabsTrigger value="sms" className="text-xs sm:text-sm gap-1 rounded-lg data-[state=active]:bg-white dark:data-[state=active]:bg-card data-[state=active]:shadow-sm">
                    <Smartphone className="h-3.5 w-3.5 hidden sm:block" />رسالة
                  </TabsTrigger>
                </TabsList>

                <TabsContent value="google" className="space-y-4 mt-5">
                  <p className="text-sm text-muted-foreground text-center">سجل الدخول بحساب Google للوصول إلى النظام</p>
                  {isInIframe ? (
                    <Button className="w-full h-12 text-base gap-3 rounded-xl bg-foreground hover:bg-foreground/90 text-background shadow-lg" onClick={handleOpenInNewTab}>
                ...
                <TabsContent value="email" className="space-y-4 mt-5">
                  <div className="space-y-2">
                    ...
                    <Input id="email" type="email" placeholder="your-email@example.com" value={email} onChange={(e) => setEmail(e.target.value)} className="ltr-input h-11 rounded-xl bg-white/60 dark:bg-card/60 backdrop-blur-sm border-border/60 focus:border-primary/50" disabled={loading} />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="password" className="text-sm font-medium">كلمة المرور</Label>
                    <Input id="password" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="h-11 rounded-xl bg-white/60 dark:bg-card/60 backdrop-blur-sm border-border/60 focus:border-primary/50" disabled={loading} />
                  </div>
                ...
                <TabsContent value="sms" className="space-y-4 mt-5">
                  <div className="space-y-2">
                    <Label htmlFor="phone" className="text-sm font-medium">رقم الهاتف</Label>
                    <Input id="phone" type="tel" placeholder="05XXXXXXXX" value={phone} onChange={(e) => setPhone(e.target.value)} className="ltr-input h-11 rounded-xl bg-white/60 dark:bg-card/60 backdrop-blur-sm border-border/60 focus:border-primary/50" disabled={loading} />
                  </div>
                  <Button className="w-full h-12 text-base gap-2 rounded-xl shadow-lg" onClick={handleSmsStart} disabled={loading || !phone}>
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-xs">
                  <span className="bg-white/70 dark:bg-card/70 backdrop-blur-sm px-3 text-muted-foreground">تسجيل دخول آمن</span>
                </div>
              </div>
